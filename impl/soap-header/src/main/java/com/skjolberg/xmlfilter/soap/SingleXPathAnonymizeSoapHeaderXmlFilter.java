...
public class SingleXPathAnonymizeSoapHeaderXmlFilter extends AbstractSingleXPathSoapHeaderXmlFilter {

	public SingleXPathAnonymizeSoapHeaderXmlFilter(boolean declaration, String expression, int filterMatches) {
		super(declaration, null, -1, -1, expression, FilterType.ANON, filterMatches);
	}
					...
					switch(chars[offset]) {
							...
							if(type == Type.INCREMENT) {
								...
								if(matches >= elementPaths.length && level == elementPaths.length) {
									if(sourceStart < offset - 1) {
										...
										if(filter.getFilterIndex() >= filterMatches) {
											return filter;
										}
									}
								}
						...
						default : {
							...
							if(matches == level && level < elementPaths.length) {
								...
								if(level == 1) {
									// check for Body
									if(matchRegion(chars, sourceStart, offset, BODY)) {
										return filter;
									}
								}
