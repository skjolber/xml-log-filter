...
/**
 * 
 * XML filter with support for max text and CDATA node sizes and a single anonymize XPath-like expression.
 * 
 * @author Thomas Rorvik Skjolberg
 *
 */

public class SingleXPathAnonymizeMaxNodeLengthXmlFilter extends SingleCharArrayXPathFilter {
	
	public SingleXPathAnonymizeMaxNodeLengthXmlFilter(boolean declaration, String expression, int maxTextNodeLength, int maxCDATANodeLength) {
		super(declaration, null, maxTextNodeLength, maxCDATANodeLength, expression, FilterType.ANON);
	}
							...
							if(chars[offset + 1] == '-') {
								...
								if((matches >= elementPaths.length && level == elementPaths.length)) {
									...
									if(offset - 6 - sourceStart - part > 0) {
										filter.add(sourceStart + 3 + part, offset - 3, -(offset - 6 - sourceStart - part));
									}
								}
